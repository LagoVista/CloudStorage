...
namespace LagoVista.CloudStorage.StorageProviders
{
    ...
    public sealed partial class MongoDocumentStorageClient : IMongoDocumentStorageClient
    {
        ...
        public async Task<DocumentStorageWriteResult> UpsertRawDocumentAsync(string entityType, string id, string json, string expectedETag = null, CancellationToken cancellationToken = default)
        {
            ...
            return new DocumentStorageWriteResult { ETag = newETag, StatusCode = result.MatchedCount > 0 ? 200 : 201, RequestCharge = null };
        }
        ...
        public async Task DeleteDocumentAsync(string entityType, string id, string partitionKey = null, CancellationToken cancellationToken = default)
        {
            ...
            if (result.DeletedCount == 0) throw new RecordNotFoundException(entityType, id);
        }
        ...
        public async Task<DocumentPage<TProjection>> GetDocumentPageAsync<TProjection>(string entityType = null, string continuationToken = null, int pageSize = 100, CancellationToken cancellationToken = default) where TProjection : class
        {
            ...
            if (!String.IsNullOrWhiteSpace(entityType)) filters.Add(Builders<BsonDocument>.Filter.Eq("EntityType", entityType));
            if (!String.IsNullOrWhiteSpace(continuationToken)) filters.Add(Builders<BsonDocument>.Filter.Gt("_id", continuationToken));
            ...
            return new DocumentPage<TProjection> { Items = items, ContinuationToken = documents.Count == pageSize ? documents.Last()["_id"].AsString : null };
        }
            ...
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity
        {
            ...
            if (!String.IsNullOrWhiteSpace(eTag)) filter &= Builders<TEntity>.Filter.Eq(entity => entity.ETag, eTag);
            item.ETag = CreateETag();
            try
            {
                var result = await GetCollection<TEntity>().ReplaceOneAsync(filter, item, new ReplaceOptions { IsUpsert = String.IsNullOrWhiteSpace(eTag) }).ConfigureAwait(false);
            ...
            {
                if (item.ETag != priorETag && !String.IsNullOrWhiteSpace(eTag)) item.ETag = priorETag;
                throw;
            }
        }
        ...
        public Task<TEntity> GetDocumentAsync<TEntity>(string id, string partitionKey, bool throwOnNotFound = true)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity => GetDocumentAsync<TEntity>(id, throwOnNotFound);

        public Task<OperationResponse<TEntity>> DeleteDocumentAsync<TEntity>(string id)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity => DeleteDocumentAsync<TEntity>(id, null);
            ...
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity
        {
            ...
            var entity = await collection.FindOneAndUpdateAsync(filter, Builders<TEntity>.Update.Combine(updates), new FindOneAndUpdateOptions<TEntity> { ReturnDocument = ReturnDocument.After }).ConfigureAwait(false);
            if (entity == null)
            {
        ...
        public Task<ListResponse<TEntity>> QueryAsync<TEntity>(Expression<Func<TEntity, bool>> query, Expression<Func<TEntity, string>> sort, ListRequest listRequest)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity => QueryAsync(query, sort, listRequest, false);
            ...
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity
        {
            ...
            var objectSort = Expression.Lambda<Func<TEntity, object>>(Expression.Convert(sort.Body, typeof(object)), sort.Parameters[0]);
            var sortDefinition = descending ? Builders<TEntity>.Sort.Descending(objectSort) : Builders<TEntity>.Sort.Ascending(objectSort);
            var items = await find.Sort(sortDefinition).Skip(Math.Max(0, listRequest.PageIndex - 1) * listRequest.PageSize).Limit(listRequest.PageSize).ToListAsync().ConfigureAwait(false);
            return ListResponse<TEntity>.Create(listRequest, items);
        }

        public Task<ListResponse<TEntity>> QueryAsync<TEntity, TKey>(Expression<Func<TEntity, bool>> query, Expression<Func<TEntity, TKey>> sort, ListRequest listRequest, bool descending)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity
        {
            if (query == null) throw new ArgumentNullException(nameof(query));
            if (sort == null) throw new ArgumentNullException(nameof(sort));
        ...
        private static async Task<ListResponse<TEntity>> CreateListResponseAsync<TEntity>(IFindFluent<TEntity, TEntity> find, ListRequest listRequest) where TEntity : class
        {
            var items = await find.Skip(Math.Max(0, listRequest.PageIndex - 1) * listRequest.PageSize).Limit(listRequest.PageSize).ToListAsync().ConfigureAwait(false);
            return ListResponse<TEntity>.Create(listRequest, items);
        }
        ...
        private static FilterDefinition<TEntity> CreatePagedQueryFilter<TEntity>(Expression<Func<TEntity, bool>> query, ListRequest listRequest)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity
        {
            var filters = new List<FilterDefinition<TEntity>> { Builders<TEntity>.Filter.Where(query), Builders<TEntity>.Filter.Eq(item => item.EntityType, typeof(TEntity).Name) };
            if (!listRequest.ShowDeleted) filters.Add(Builders<TEntity>.Filter.Or(Builders<TEntity>.Filter.Exists("IsDeleted", false), Builders<TEntity>.Filter.Eq("IsDeleted", false)));
            if (!listRequest.ShowDrafts) filters.Add(Builders<TEntity>.Filter.Or(Builders<TEntity>.Filter.Exists("IsDraft", false), Builders<TEntity>.Filter.Eq("IsDraft", false)));
        ...
        public async Task<IEnumerable<TResult>> QueryKnownAsync<TResult>(string entityType, DocumentQueryRequest request, CancellationToken cancellationToken = default) where TResult : class
        {
            ...
            switch (request.QueryType)
            {
                ...
                case DocumentQueryType.EntityUtilsDocumentsByFieldValue: return await QueryEntityUtilsByFieldValueAsync<TResult>(collection, request, cancellationToken).ConfigureAwait(false);
                case DocumentQueryType.EntityUtilsDocumentsByStatusIds: return await QueryEntityUtilsByStatusIdsAsync<TResult>(collection, request, cancellationToken).ConfigureAwait(false);
                case DocumentQueryType.EntityUtilsDocumentsWithEmptyField: return await QueryEntityUtilsWithEmptyFieldAsync<TResult>(collection, request, cancellationToken).ConfigureAwait(false);
                case DocumentQueryType.CustomerIndustryNicheSalesStageCounts:
                    return Deserialize<TResult>(await collection.Aggregate<BsonDocument>(new BsonDocument[] { new BsonDocument("$match", new BsonDocument { { "EntityType", "CustomerEntity" }, { "OwnerOrganization.Id", request.GetRequired<string>("orgId") } }), new BsonDocument("$group", new BsonDocument { { "_id", new BsonDocument { { "Industry", "$Industry" }, { "IndustryNiche", "$IndustryNiche" }, { "SalesStage", "$SalesStage" } } }, { "CountLeads", new BsonDocument("$sum", 1) } }), new BsonDocument("$project", new BsonDocument { { "_id", 0 }, { "Industry", "$_id.Industry" }, { "IndustryNiche", "$_id.IndustryNiche" }, { "SalesStage", "$_id.SalesStage" }, { "CountLeads", 1 } }) }).ToListAsync(cancellationToken).ConfigureAwait(false));
