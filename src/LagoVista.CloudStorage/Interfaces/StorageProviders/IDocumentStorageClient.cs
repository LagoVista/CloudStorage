...
namespace LagoVista.CloudStorage.Interfaces
{
    ...
    public interface IDocumentStorageClient
    {
        ...
        Task<IEntityBase> GetDocumentAsync(Type entityType, string entityTypeName, string id, bool throwOnNotFound = true, CancellationToken cancellationToken = default);
        ...
        Task<InvokeResult> PatchDocumentAsync(Type entityClrType, string entityType, PatchRequest request, CancellationToken cancellationToken = default);
        ...
        Task<ListResponse<TEntity>> QueryAllAsync<TEntity, TKey>(Expression<Func<TEntity, bool>> query, Expression<Func<TEntity, TKey>> sort, ListRequest listRequest, bool descending)
            where TEntity : class, IIDEntity, IKeyedEntity, IOwnedEntity, INamedEntity, INoSQLEntity, IAuditableEntity;
