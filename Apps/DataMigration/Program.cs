...
{
    var tableSizer = new TableStoragePruner(syncSettings, _logger);
    ...
    var stats = await tableSizer.RunAsync(options, sampleSizePerTable: 500, maxConcurrency: 6, ct);
    ...
    foreach(var table in stats.Where(st=>st.WouldDelete))
    {
        ...
        {
            var result = await tableSizer.PruneTableAsync(table.TableName, ct);
